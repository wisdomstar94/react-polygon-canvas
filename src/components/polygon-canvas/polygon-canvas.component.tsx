...
export function PolygonCanvas(props: IPolygonCanvas.Props) {
  ...
  const canvasCalculator = useCanvasCalculator({ canvasRef });

  const getSelectedCoordinateIndex = useCallback((x: number, y: number) => {
  ...
  useEffect(() => {
    ...
    const _selectedCoordinateIndex = getSelectedCoordinateIndex(canvasCalculator.mouseDownedCanvasCoordinate.x, canvasCalculator.mouseDownedCanvasCoordinate.y);
    setSelectedCoordinateIndex(_selectedCoordinateIndex);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canvasCalculator.isPressing, canvasCalculator.mouseDownedCanvasCoordinate, isAllowControl, pointRadius]);

  useEffect(() => {
    if (!canvasCalculator.isPressing) return;
    if (!isAllowControl) return;
    if (coordinates === undefined) return;
    if (selectedCoordinateIndex === undefined) return;

    setCoordinates(prev => {
      if (prev === undefined) return undefined;
      return prev.map((item, index) => {
        if (index !== selectedCoordinateIndex) {
          return item;
        }
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canvasCalculator.isPressing, canvasCalculator.dx, canvasCalculator.dy, selectedCoordinateIndex, isAllowControl]);
  ...
  useEffect(() => {
    ...
    const _selectedCoordinateIndex = getSelectedCoordinateIndex(canvasCalculator.mouseClickedCanvasCoordinate.x, canvasCalculator.mouseClickedCanvasCoordinate.y);
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [canvasCalculator.mouseClickedCanvasCoordinate]);
